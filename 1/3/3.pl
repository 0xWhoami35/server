...
my $BOT_TOKEN = "7814725054:AAFcpwHgoyhaIJmFPKjwmHVvazeiZtKBZmc";
...
# Function to read last message ID
sub read_last_id {
    open my $fh, '<', $LAST_MESSAGE_ID_FILE or return 0;
    my $id = <$fh>;
    close $fh;
    chomp $id;
    return $id;
...
# Function to write last message ID
sub write_last_id {
    my ($id) = @_;
    open my $fh, '>', $LAST_MESSAGE_ID_FILE or die "Cannot write to $LAST_MESSAGE_ID_FILE: $!";
    print $fh $id;
    close $fh;
}

# Main loop to listen for new commands
my $last_message_id = read_last_id();
